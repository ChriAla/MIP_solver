...
class FacilityLocationProblem:
    ...
    def create_gui(self):
        ...
        Label(self.root, text="Number of Facilities:").grid(row=0, column=0, sticky="w")
        self.entry_num_facilities = Entry(self.root)
        self.entry_num_facilities.grid(row=0, column=1)

        Label(self.root, text="Number of Clients:").grid(row=1, column=0, sticky="w")
        self.entry_num_clients = Entry(self.root)
        self.entry_num_clients.grid(row=1, column=1)

        Label(self.root, text="Facility Opening Costs (space-separated):").grid(row=2, column=0, sticky="nw")
        self.text_facility_costs = Text(self.root, height=5, width=40)
        self.text_facility_costs.grid(row=2, column=1)

        Label(self.root, text="Transport Costs (space-separated rows):").grid(row=3, column=0, sticky="nw")
        self.text_transport_costs = Text(self.root, height=10, width=40)
        self.text_transport_costs.grid(row=3, column=1)

        # Κουμπί επίλυσης
        solve_button = Button(self.root, text="Solve the problem", command=self.solve_facility_location)
        solve_button.grid(row=4, column=0, columnspan=2)

    def solve_facility_location(self):
        try:
            # Ανάγνωση δεδομένων από το GUI
            num_facilities = int(self.entry_num_facilities.get())
            num_clients = int(self.entry_num_clients.get())
            facility_costs_input = self.text_facility_costs.get("1.0", END).strip()
            transport_costs_input = self.text_transport_costs.get("1.0", END).strip()
