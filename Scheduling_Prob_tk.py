...
class SchedulingProblem:
    ...
    def create_gui(self):
        ...
        Label(self.root, text="Number of Tasks:").grid(row=0, column=0, sticky="w")
        self.entry_num_tasks = Entry(self.root)
        self.entry_num_tasks.grid(row=0, column=1)

        Label(self.root, text="Number of Machines:").grid(row=1, column=0, sticky="w")
        self.entry_num_machines = Entry(self.root)
        self.entry_num_machines.grid(row=1, column=1)

        Label(self.root, text="Task Durations (space-separated):").grid(row=2, column=0, sticky="nw")
        self.text_task_durations = Text(self.root, height=5, width=40)
        self.text_task_durations.grid(row=2, column=1)

        # Κουμπί επίλυσης
        solve_button = Button(self.root, text="Solve the problem", command=self.solve_scheduling)
        solve_button.grid(row=3, column=0, columnspan=2)

    def solve_scheduling(self):
        try:
            # Ανάγνωση δεδομένων από το GUI
            num_tasks = int(self.entry_num_tasks.get())
            num_machines = int(self.entry_num_machines.get())
            task_durations_input = self.text_task_durations.get("1.0", END).strip()
