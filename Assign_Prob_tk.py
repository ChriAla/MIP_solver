...
class AssignmentProblem:
    ...
    def create_gui(self):
        ...
        Label(self.root, text="Number of Workers:").grid(row=0, column=0, sticky="w")
        self.entry_num_workers = Entry(self.root)
        self.entry_num_workers.grid(row=0, column=1)

        Label(self.root, text="Number of Jobs:").grid(row=1, column=0, sticky="w")
        self.entry_num_tasks = Entry(self.root)
        self.entry_num_tasks.grid(row=1, column=1)

        Label(self.root, text="Cost Table (space-separated lines):").grid(row=2, column=0, sticky="nw")
        self.text_costs = Text(self.root, height=10, width=40)
        self.text_costs.grid(row=2, column=1)

        # Κουμπί επίλυσης
        solve_button = Button(self.root, text="Solve the problem", command=self.solve_assignment)
        solve_button.grid(row=3, column=0, columnspan=2)

    def solve_assignment(self):
        try:
            # Ανάγνωση δεδομένων από το GUI
            num_workers = int(self.entry_num_workers.get())
