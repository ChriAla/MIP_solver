...
class VehicleRoutingProblem:
    ...
    def create_gui(self):
        ...
        Label(self.root, text="Number of Customers:").grid(row=0, column=0, sticky="w")
        self.entry_num_customers = Entry(self.root)
        self.entry_num_customers.grid(row=0, column=1)

        Label(self.root, text="Number of Vehicles:").grid(row=1, column=0, sticky="w")
        self.entry_num_vehicles = Entry(self.root)
        self.entry_num_vehicles.grid(row=1, column=1)

        Label(self.root, text="Vehicle Capacity:").grid(row=2, column=0, sticky="w")
        self.entry_vehicle_capacity = Entry(self.root)
        self.entry_vehicle_capacity.grid(row=2, column=1)

        Label(self.root, text="Demand per Customer (space-separated):").grid(row=3, column=0, sticky="nw")
        self.text_demand = Text(self.root, height=5, width=40)
        self.text_demand.grid(row=3, column=1)

        Label(self.root, text="Cost Matrix (space-separated rows):").grid(row=4, column=0, sticky="nw")
        self.text_costs = Text(self.root, height=10, width=40)
        self.text_costs.grid(row=4, column=1)

        # Κουμπί επίλυσης
        solve_button = Button(self.root, text="Solve the problem", command=self.solve_vrp)
        solve_button.grid(row=5, column=0, columnspan=2)

    def solve_vrp(self):
        try:
            # Ανάγνωση δεδομένων από το GUI
            num_customers = int(self.entry_num_customers.get())
            num_vehicles = int(self.entry_num_vehicles.get())
            capacity = int(self.entry_vehicle_capacity.get())
            demand_input = self.text_demand.get("1.0", END).strip()
            cost_input = self.text_costs.get("1.0", END).strip()
